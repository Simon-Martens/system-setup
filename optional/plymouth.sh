...
if ! command -v plymouth &>/dev/null; then
  ...
  if [ -d "/boot/loader/entries" ]; then
    ...
    if ! grep -q splash /etc/cmdline.d/*.conf; then
        # Need splash, create the system-setup file
        echo "splash" | sudo tee -a /etc/cmdline.d/system-setup.conf
    fi
    if ! grep -q quiet /etc/cmdline.d/*.conf; then
        # Need quiet, create or append the system-setup file
        echo "quiet" | sudo tee -a /etc/cmdline.d/system-setup.conf
    fi
  ...
  sudo cp -r "$HOME/.local/share/system-setup/default/plymouth" /usr/share/plymouth/themes/system-setup/

  sudo plymouth-set-default-theme -R system-setup
fi
